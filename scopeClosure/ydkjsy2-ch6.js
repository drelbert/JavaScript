...
designPrinciple = {
    ...
    for: "building a defensive posture into the architecture = component function with least access"
};
...
function diff(x, y) {
  ...
  practice = "hide the variables and function declarations in the lowest scope (deeply nested) possible"
  ...
  curlyBracePair = "{..} which is a statement that acts like a block, not necc as a scope"

  blockToScope = "block to scope only occurs if a need to contain block-scoped declarations let or const";
  ...
  // In practice 
  {
      ...
      for (let i = 0; i < 5; i++) { // Also a scope activated each iteration
        if ( i % 2 == 0) {
            // Only a block
            console.log(i);
        }
  ...
  explicitBlockScoping = "for creating a thin slice of scope for a couple or less variables";

  // Illustration
  if (someAction) {
      // Block but not a scope
      {
          let msg = someAction.message();  // This is both a block and en explicit scope
          notifyVader(msg);
      }
...
ask = " What is the most minimal scope exposure that is sufficent for this variable?"

answerWill = "define if a variable belongs in a block scope or the function scope"
...
fromES3 = "catch clause has used an additional block scope declaration cabability"

neInES2019 = "catch clauses changed do their declaration is optional, if omitted, the catch block is longer by default a scope, but still a block"
...
// Consider 
if (false) {
    function ask() {  // JS Spec says this function declaration is block-scoped
