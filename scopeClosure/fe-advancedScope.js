...
var aboutLexicalScope = [
  ...
  {
    content:
      "authorTime: fixed at author time to put one function inside another",
    ruleNumber: 3,
    confirmed: true,
  },
  {
...
point =
  "when variable is exposed, someone will use it = now restricted in ability to refactor";
...
(function anotherTeacher() {
  // Not a function declaration but a function expression, since function is not the first item in the statement
  var teacher = "Lupe";
  console.log(teacher);
...
aboutBlocks = {
  ...
  not: "expressions",
  noVar:
    "use let (or const) for making a declaration inside a block, turning the block into a scope",
...
howBlockScopeFits = {
  ...
  useOnly:
    "where it already makes sence to block scope (maybe where var is already)",
};
...
function repeat(fn, n) {
  ...
  for (let i = 0; i < n; i++) {
    // This let was origininally a var
    result = fn(result, i);
  }
  return result;
}
...
detailsConst = {
  ...
  use:
    "when assigning a value that is already a primitive immutable values: string, number, boolean.  Use as sematic placeholder for literals",
};
...
notAThing =
  "metaphor to discuss the idea of lexical scope, without getting detailed about it";
usedFor =
  "describing the parse and execute phase without getting into the details";
