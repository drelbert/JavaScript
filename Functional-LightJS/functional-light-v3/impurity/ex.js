...
function getStudentsByName(students) {
  // the wrapping function to limit impurity
  // passing in the original list of students
  students = students.slice(); // making the local reference of students a copy, not a reference to the original
  // slice() makes a shallow copy of the array
  // with the above line the paramter is pointing at a local copy, not the outer

  return sortStudentsByName();
  // this produces a side effect on the local copy not the global one

  function sortStudentsByName() {
    // impure function given the .sort() so must be contained by wrapping it with the getStudentsByName() function
...
function getStudentsByID(curStudents) {
  var origStudents = students.slice(); // step one: saving the original state by making a backup/local copy of students
  students = curStudents.slice(); // step two: set up initial state for students
  var newStudents = sortStudentsByID(); //steps three and four: call to make the modification and capture the new value
  students = origStudents; // step five: restore the original state
  return newStudents; // step six: return the new state that was captured
}

// *************************************

// set of tests, all need to output true
